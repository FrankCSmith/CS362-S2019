...
int main(){
	...
	//Expect to receive 1 card and 1 action, run cardEffect
	int addedCards = 1;
	int addedActions = 1;
	...
	//Check that players deck has decreased by 1 cards, it came from own pile
	...
	//Check that 1 action added
	int GNumActions = G.numActions;
	int testGNumActions = testG.numActions;

	printf("Correct Number of Actions Added\n:");
	printf("Expected actions count = %d\n", testGNumActions + 1);
	printf("Actual actions count = %d\n", GNumActions);
	if (GNumActions == testGNumActions)
		printf("PASSED\n");
	else{
		printf("FAILED\n");
		passFlag = 1;
	...
	printf("Card Test 4: great_hall result: ");
	if (passFlag == 0)
		printf("PASSED\n");
	else
		printf("FAILED\n");
